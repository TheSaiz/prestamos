...
async function sendMessage() {
    ...
    if (file) {
        ...
        formData.append("message", message || ""); // <-- enviamos el texto también
        ...
        if (data.success) {
            addFileToChat({
                ...
                size: file.size,
                text: message || ""       // <-- mostramos el texto junto con el archivo
            });
        } else {
...
function addFileToChat(file) {
    ...
    if (isAsesor) {
        ...
        // Archivo enviado por el cliente (izquierda, blanco)
        div.innerHTML = `
            <div class="flex gap-2 max-w-[60%]">
                <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                    <span class="text-white text-xs font-semibold">C</span>
                ...
                <div>
                    <div class="bg-white rounded-2xl rounded-tl-none px-4 py-2.5 shadow-sm">
                        ${file.text ? `<p class="text-sm text-gray-800 mb-2">${escapeHtml(file.text)}</p>` : ""}
                        <a href="${file.url}" target="_blank"
...
// REEMPLAZAR el bloque de POLLING MENSAJES NUEVOS en panel_asesor.php
