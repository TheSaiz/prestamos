...
async function cerrarChat() {
    ...
    try {
        // 1. Enviar mensaje de sistema
        const formDataMsg = new FormData();
        formDataMsg.append('chat_id', chatId);
        formDataMsg.append('sender', 'asesor');
        formDataMsg.append('message', '❌ Chat cerrado por el asesor');
        ...
        formDataClose.append('chat_id', chatId);
...
// ===================================================================
// POLLING CHATS PENDIENTES - SISTEMA COMPETITIVO (CORREGIDO)
// ===================================================================
...
setInterval(async () => {
    try {
        ...
        if (data.success && data.data.chats.length > 0) {
            ...
            // Actualizar badge
            badge.textContent = `${pendingChats.length} chat${pendingChats.length > 1 ? 's' : ''} pendiente${pendingChats.length > 1 ? 's' : ''}`;
            ...
            // 🔥 ELIMINAR notificaciones que ya no están en la lista
            ...
            pendingChats.forEach((chat, index) => {
                ...
                // 👉 SI ES NUEVO → CREAR NOTIFICACIÓN
                ...
                // 👉 SI YA EXISTE → SOLO ACTUALIZAR ESTADO
                    ...
                    updateNotificationState(chat.id, isFirst);
                }
            });

        } else {
            // 🧹 NO HAY CHATS → LIMPIAR TODO
...
// ===================================================================
// MOSTRAR NOTIFICACIÓN DE NUEVO CHAT (CORREGIDO)
// ===================================================================
...
function showNewChatNotification(chat, isFirst) {
    ...
    // ⚠️ SI YA EXISTE → NO RECREAR, SOLO ACTUALIZAR
    if (existingNotif) {
        console.warn(`⚠️ Notificación ${chat.id} ya existe, no se recrea`);
        return;
    }
    ...
    notif.innerHTML = `
        <div class="flex items-start gap-3">
            <span class="material-icons-outlined text-blue-600 pulse-icon">notifications_active</span>
            <div class="flex-1">
    ...
    // 🔊 Sonido solo para el primero
    if (isFirst) {
        playNotificationSound();
    }

    console.log(`✅ Notificación creada para chat ${chat.id}`);
}
...
// ===================================================================
// ACTUALIZAR ESTADO DE NOTIFICACIÓN EXISTENTE
// ===================================================================

function updateNotificationState(chatId, isFirst) {
    const notif = document.getElementById('notif-' + chatId);
    if (!notif) return;

    const actionsContainer = notif.querySelector(`#notif-actions-${chatId}`);
    if (!actionsContainer) return;

    // Actualizar botón según posición en cola
    actionsContainer.innerHTML = isFirst
        ? `<button onclick="acceptChat(${chatId})"
                class="w-full px-3 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg text-sm font-semibold hover:from-green-600 hover:to-green-700 transition shadow-md flex items-center justify-center gap-2">
            <span class="material-icons-outlined text-sm">check_circle</span>
            Aceptar Chat
           </button>`
    ...
    // Agregar efecto visual si pasó a ser el primero
    if (isFirst) {
