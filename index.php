...
async function uploadFile(file) {
    if (!chatId) return alert("Debes iniciar el chat primero");

    const temp = document.createElement("div");
    temp.className = "flex justify-end mb-2";
    temp.innerHTML = `<div class="upload-progress">Subiendo ${escapeHtml(file.name)}… ⏳</div>`;
...
document.getElementById("start-chat-btn").addEventListener("click", async () => {
    ...
    try {
        // ---------------------------------------------
        // 1. VALIDAR DNI — MISMA API DEL FORMULARIO
        // ---------------------------------------------
        const validarResponse = await fetch("https://prestamolider.com/system/api/chatbot/validar_dni.php", {
            method: "POST",
        ...
        if (!validarData.success) {
            alert(validarData.message || "No se pudo validar el DNI");
            return;
        }

        // Si vienen varios CUIT, mostrar selección (igual que el form original)
if (validarData.casos > 1) {
    let opciones = validarData.opciones
        .map(x => `${x.nombre} (${x.cuil})`)
        .join("\n");
    ...
    if (!elegido) {
        ...
        // Si llegó aquí → el DNI es válido
        console.log("DNI validado correctamente");

        // ---------------------------------------------
        // 2. AHORA SÍ, INICIAR EL CHAT
        // ---------------------------------------------
        ...
        // Guardamos IDs retornados por el backend
        ...
        addBotMessage(`¡Hola ${nombre}! 😊 Vamos a comenzar tu solicitud.`);
    ...
    msg.innerHTML = `
        <div class="max-w-[80%] flex gap-2">
            ...
            <div class="bg-gray-800 text-white rounded-2xl px-4 py-2.5 inline-block">
                ${fileData.text ? `<p class="text-sm mb-2">${escapeHtml(fileData.text)}</p>` : ""}
