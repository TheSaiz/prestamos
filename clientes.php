...
// Total de clientes ÚNICOS (no conversaciones)
$stmt = $pdo->query("SELECT COUNT(DISTINCT cliente_id) FROM chats WHERE origen = 'chatbot'");
$metricas['total'] = $stmt->fetchColumn();

// Por estado de solicitud (última conversación de cada cliente)
$stmt = $pdo->query("
    SELECT estado_solicitud, COUNT(*) as total
    FROM chats c
    INNER JOIN (
        SELECT cliente_id, MAX(id) as ultimo_chat_id
        FROM chats
        WHERE origen = 'chatbot'
        GROUP BY cliente_id
    ) ultimos ON c.id = ultimos.ultimo_chat_id
    WHERE origen = 'chatbot'
    GROUP BY estado_solicitud
");
$metricas['por_estado'] = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);

// Nuevos hoy (clientes únicos)
$stmt = $pdo->query("
    SELECT COUNT(DISTINCT cliente_id) FROM chats
    WHERE origen = 'chatbot' AND DATE(fecha_inicio) = CURDATE()
");
$metricas['hoy'] = $stmt->fetchColumn();

// Pendientes de contacto (última conversación)
$stmt = $pdo->query("
...
// Contar total de registros (clientes únicos)
$stmt = $pdo->prepare("
...
// =====================================================
// OBTENER CLIENTES CON TODA LA INFO (AGRUPADOS)
// =====================================================
$stmt = $pdo->prepare("
    SELECT
        c.id as chat_id,
        ...
        (SELECT COUNT(*) FROM chats WHERE cliente_id = u.id AND origen = 'chatbot') as total_conversaciones,
        (SELECT MIN(fecha_inicio) FROM chats WHERE cliente_id = u.id AND origen = 'chatbot') as primera_interaccion
    FROM chats c
    INNER JOIN usuarios u ON c.cliente_id = u.id
    LEFT JOIN clientes_detalles cd ON cd.usuario_id = u.id
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Interacciones</th>
                        ...
                        <?php else: ?>
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2">
                                            <span class="material-icons-outlined text-blue-600 text-sm">chat_bubble</span>
                                            <span class="text-sm font-semibold text-gray-800"><?php echo (int)$cliente['total_conversaciones']; ?> conversación<?php echo (int)$cliente['total_conversaciones'] !== 1 ? 'es' : ''; ?></span>
                                        </div>
                                        <p class="text-xs text-gray-500">
                                            Última: <?php echo date('d/m/Y H:i', strtotime($cliente['fecha_inicio'])); ?>
                                        </p>
                                        <?php if ($cliente['primera_interaccion'] !== $cliente['fecha_inicio']): ?>
                                        <p class="text-xs text-gray-400">
                                            Primera: <?php echo date('d/m/Y', strtotime($cliente['primera_interaccion'])); ?>
                                        </p>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="cliente_detalle.php?id=<?php echo (int)$cliente['chat_id']; ?>"
                                           ...
                                           title="Ver conversación más reciente">
                                            <span class="material-icons-outlined text-sm mr-1">visibility</span>
                                            Ver
                                        </a>
                                        <?php if ((int)$cliente['total_conversaciones'] > 1): ?>
                                        <a href="cliente_historial.php?cliente_id=<?php echo (int)$cliente['cliente_id']; ?>"
                                           ...
                                           title="Ver todas las conversaciones">
                                            <span class="material-icons-outlined text-sm mr-1">history</span>
                                            Historial
                                        </a>
                                        <?php endif; ?>
                                    </div>
            ...
            <div class="bg-gray-50 px-6 py-4 flex items-center justify-between border-t">
                <div class="text-sm text-gray-600">
                    ...
                    de <?php echo $total_registros; ?> clientes únicos
                </div>
                <div class="flex gap-2">
