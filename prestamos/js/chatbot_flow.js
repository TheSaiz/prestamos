...
// RUTA: /system/js/chatbot_flow.js
// ===================================================================
...
async function handleUserInput(text) {
    ...
    if (q.id === 2) {
        ...
        try {
            ...
            confirmar(
                `¿Sos ${d.nombre}?`,
                async () => {
                    await saveAnswer(q.id, d.nombre, 0, d.cuil, d.nombre);
                    await avanzar();
                },
                resetChatbot
            );
    ...
    if (q.pregunta.includes("código de área")) {
        ...
        if (!/^\d{1,3}$/.test(limpio)) {
            botSay("❌ Ingresá hasta 3 dígitos numéricos");
            return;
        }
...
async function finalizarChatbot() {
    try {
        await fetch("/system/api/chatbot/mark_waiting_asesor.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
// GUARDAR RESPUESTA
// ===================================================================
async function saveAnswer(questionId, answer, optionId = 0, cuil = "", nombre = "") {
