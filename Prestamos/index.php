...
<body class="bg-gray-50">
   ...
   <section class="relative min-h-screen flex items-center bg-hero-gradient">
    ...
    let clienteId = null;           // ID del cliente
    let currentQuestionId = null;   // ID de la pregunta actual
    let departamentoId = null;      // Departamento seleccionado
    let waitingForText = false;     // Si estamos esperando respuesta de texto
        ...
        try {
            ...
            const response = await fetch("api/chat/start_chat.php", {
                ...
                body: JSON.stringify({
                    ...
                    dni: dni,
                    telefono: telefono,
                    departamento_id: 1 // Temporal, se actualizará con la respuesta
                })
            });
    ...
    async function getNextQuestion() {
        try {
            ...
            // Si es de tipo opción, mostrar botones
            ...
            // Si es de tipo texto, habilitar input
    ...
    async function saveAnswer(answer, optionId = null) {
        try {
            ...
            // Si detectó un departamento, guardarlo
    ...
    // -------------------------------------------------------------------
    // ENVIAR MENSAJE DE TEXTO (cuando es pregunta tipo "texto")
    // -------------------------------------------------------------------
    ...
    async function finalizarChatbot() {
        ...
        try {
            // Notificar a asesores disponibles del departamento
            ...
            if (data.success) {
                if (data.data.notificados.length > 0) {
                    ...
                    // Iniciar polling para verificar si un asesor aceptó
                    iniciarPollingAsesor();
                } else {
                    addMessage("⚠️ No hay asesores disponibles en este momento. Por favor intenta más tarde o llámanos al <?php echo $config['contact']['phone']; ?>");
                }
            }

        } catch (error) {
            console.error("Error:", error);
            addMessage("Error al conectar con asesores. Por favor contacta directamente al <?php echo $config['contact']['phone']; ?>");
        }
    }
