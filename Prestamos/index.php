...
<head>
    ...
    <script src="js/chatbot_flow.js"></script>
...
document.getElementById("start-chat-btn").addEventListener("click", async () => {
    ...
    try {
        const response = await fetch("http://localhost/prestamos/api/chat/start_chat.php", {
            method: "POST",
        ...
        const data = await response.json();
        if (!data.success) return alert("Error al iniciar el chat");
...
chatInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        e.preventDefault(); // evita que dispare el click también
        enviarMensajeTexto();
    }
});
...
async function enviarMensajeTexto() {
    ...
    // Desactivo temporalmente hasta que llegue la próxima pregunta
    waitingForText = false;

    await handleUserInput(text);

    // Reactivar cuando corresponda SOLO si el chatbot lo pide
    // waitingForText = true;
