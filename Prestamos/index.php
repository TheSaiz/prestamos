...
function toggleFAQ(i) {
    ...
    <div id="chat-widget-window">
        ...
        <div id="chat-input-area" style="display: none;">
            ...
            <input type="file" id="chat-file-input" accept=".pdf,.png,.jpg,.jpeg,.webp,.xls,.xlsx" style="display: none;">
    ...
    <script>
        ...
        chatFileInput.addEventListener("change", async (e) => {
            ...
            if (!allowedTypes.includes(file.type)) {
                alert("Tipo de archivo no permitido. Solo: PDF, PNG, JPG, JPEG, WEBP, XLS, XLSX");
                chatFileInput.value = "";
                return;
            }
        ...
        async function uploadFile(file) {
            ...
            try {
                ...
                progressDiv.className = "flex justify-end mb-4";
                ...
                if (data.success) {
                    addFileMessage(file.name, formatFileSize(file.size), data.data.archivo_url, "user");
                } else {
        ...
        function addFileMessage(fileName, fileSize, fileUrl, sender = "bot") {
    ...
    if (sender === "user") {
        msgDiv.innerHTML = `
            <div class="max-w-[80%]">
                <div class="bg-gradient-to-r from-blue-600 to-green-600 text-white rounded-2xl rounded-br-none px-4 py-2.5 shadow-sm inline-block">
                    <p class="text-sm mb-2">Archivo enviado:</p>
                    <a href="${fileUrl}" target="_blank" class="msg-file" style="background: rgba(255,255,255,0.2); border-color: rgba(255,255,255,0.4);">
                        <span class="file-icon">${fileIcon}</span>
                        <div class="file-info">
                            <span class="file-name" style="color: white;">${escapeHtml(fileName)}</span>
                            <span class="file-size" style="color: rgba(255,255,255,0.8);">${fileSize}</span>
                        </div>
                    </a>
                </div>
        ...
        msgDiv.innerHTML = `
            <div class="max-w-[80%]">
                <div class="bg-gray-800 text-white rounded-2xl rounded-bl-none px-4 py-2.5 shadow-sm inline-block">
                    <p class="text-sm mb-2">Archivo compartido:</p>
                    <a href="${fileUrl}" target="_blank" class="msg-file" style="background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.3);">
                        <span class="file-icon">${fileIcon}</span>
                        <div class="file-info">
                            <span class="file-name" style="color: #dbeafe;">${escapeHtml(fileName)}</span>
                            <span class="file-size" style="color: #9ca3af;">${fileSize}</span>
                        </div>
                    </a>
                </div>
                ...
                data.data.messages.forEach(msg => {
                    if (msg.sender === "asesor" || msg.sender === "bot") {
                        ...
                        if (msg.tiene_archivo && msg.archivo) {
                            ...
                            addFileMessage(
                                msg.archivo.nombre,
                                formatFileSize(msg.archivo.tamano),
                                msg.archivo.url,
                                "bot"
                            );
                        } else {
