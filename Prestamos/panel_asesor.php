...
if (chatId) {
        ...
        try {
            ...
            if (data.success && data.data.messages.length > 0) {
                ...
                data.data.messages.forEach(msg => {
                    ...
                    // --- SI EL CLIENTE FINALIZÓ LA CONVERSACIÓN ---
                    if (
                        msg.sender === 'cliente' &&
                        msg.message &&
                    ...
                    if (msg.tiene_archivo && msg.archivo) {
                        addFileToChat({
                            ...
                            text: msg.message || ""   // <--- AHORA SÍ SE MUESTRA EL TEXTO + ARCHIVO
                        });
                    }
    ...
    // Obtener la lista actualizada de pendientes
    const resp = await fetch(`api/asesores/get_pending_chats.php?asesor_id=${asesorId}`);
    const data = await resp.json();

    // Verificar si este chat es el PRIMERO de la cola
    ...
    // Reproducir sonido solo si es el primero
    if (isFirst) {
        try {
