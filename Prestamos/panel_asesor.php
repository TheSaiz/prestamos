...
// Obtener chats agrupados por DNI (historial unificado)
$stmt = $pdo->prepare("
    SELECT 
        cd.dni,
...
if ($chat_activo_id) {
    ...
    // Obtener historial completo por DNI
    $stmt = $pdo->prepare("
        SELECT m.*, 
    ...
    // Marcar todos los chats del cliente como leídos
    $stmt = $pdo->prepare("
        UPDATE chats 
...
<body class="bg-gray-50 h-screen overflow-hidden">
   ...
   class="chat-item block px-4 py-3 border-b border-gray-100 cursor-pointer transition 
          <?php echo ($chat_activo_id == $chat['chat_id_representativo']) ? 'active' : ''; ?>">
...
<?php endif; ?>
    ...
    <div id="profile-panel"
     class="w-80 bg-white border-l border-gray-200 overflow-y-auto flex-none min-h-0">
        ...
        try {
            ...
            if (data.success && data.data.messages.length > 0) {
                data.data.messages.forEach(msg => {
                    ...
                    // Detectar si el cliente finalizó la conversación
if (
    msg.sender === 'cliente' &&
...
// Agregar mensaje entrante del cliente
if (msg.sender === 'cliente') {
    addMessageToChat(msg.message, 'cliente');
}

// Actualizar último ID recibido
